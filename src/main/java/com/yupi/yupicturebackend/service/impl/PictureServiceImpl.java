...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            if (!canUploadToSpace(loginUser, space)) {
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        pictureQueryRequest.setSortField(pictureQueryRequest.getSortField());
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        if (PictureReviewStatusEnum.REVIEWING.equals(reviewStatusEnum) || id == null || reviewStatusEnum == null) {
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "最多 30 张");
        // 名称前缀默认等于搜索关键词
    ...
    public void checkPictureViewAuth(User loginUser, Picture picture) {
        ...
        Space space = spaceService.getById(spaceId);
        ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
        if (!canViewSpace(loginUser, space)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
    }
    ...
    public void checkPictureAuth(User loginUser, Picture picture) {
        ...
        if (spaceId == null) {
            ...
            Space space = spaceService.getById(spaceId);
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
            if (!canManagePicture(loginUser, picture, space)) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
            }
        }
    }

    private boolean canViewSpace(User loginUser, Space space) {
        Long loginUserId = loginUser.getId();
        if (loginUserId.equals(space.getUserId())) {
            return true;
        }
        return SpaceTypeEnum.TEAM.getValue() == space.getSpaceType()
                && spaceUserService.getBySpaceIdAndUserId(space.getId(), loginUserId) != null;
    }

    private boolean canUploadToSpace(User loginUser, Space space) {
        Long loginUserId = loginUser.getId();
        if (loginUserId.equals(space.getUserId())) {
            return true;
        }
        ...
        SpaceUser spaceUser = spaceUserService.getBySpaceIdAndUserId(space.getId(), loginUserId);
        if (spaceUser == null) {
            return false;
        }
        String spaceRole = spaceUser.getSpaceRole();
        return SpaceRoleEnum.ADMIN.getValue().equals(spaceRole)
                || SpaceRoleEnum.EDITOR.getValue().equals(spaceRole);
    }

    private boolean canManagePicture(User loginUser, Picture picture, Space space) {
        Long loginUserId = loginUser.getId();
        if (loginUserId.equals(space.getUserId()) || loginUserId.equals(picture.getUserId())) {
            return true;
        }
        if (SpaceTypeEnum.TEAM.getValue() != space.getSpaceType()) {
            return false;
